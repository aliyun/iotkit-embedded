 ...
 * Some test switch
 */
#define TEST_ERROR_PK   0           /* subDev use error ProductKey if defined 1 */
#define TEST_ERROR_DN   0           /* subDev use error DeviceName if defined 1 */
#define TEST_ERROR_DS   0           /* subDev use error DeviceSecret if defined 1 */
#define TEST_CYCLE_RUN  0           /* run cycle always if defined 1 */
#define TEST_RAW_DATA   0           /* raw data upstream and downstream test if defined 1 */
#define TEST_SERVICE    0           /* service invoke test if defined 1 */


#if (TEST_RAW_DATA == 1)
    #define SUBDEV_PRODUCTKEY       "a1LE4KDqXRw"
    #define SUBDEV_DEVICENAME       "type-001"
    #define SUBDEV_DEVICESECRET     "Ed41iFclZBCOsUrF4E6lacrhn3odO7xM"
#elif (TEST_SERVICE == 1)
    #define SUBDEV_PRODUCTKEY       "a1X2bEnP82z"
    #define SUBDEV_DEVICENAME       "example1"
    #define SUBDEV_DEVICESECRET     "ga7XA6KdlEeiPXQPpRbAjOZXwG8ydgSe"
#else
...
static ssize_t gateway_rawdata_down(const void *in, int in_len, void *out, int out_len, void *ctx);
...
static linkkit_cbs_t linkkit_cbs = {
    ...
    .down_rawdata = gateway_rawdata_down
};
...
static int gateway_call_service(char *identifier, char *in, char *out, int out_len, void *ctx)
{
     ...
     */
    EXAMPLE_TRACE("Service invoke handler, service is %s\n", identifier);
    EXAMPLE_TRACE("input = %s\n", in);

    if (strstr(identifier, "Custom") != 0) {
        snprintf(out, out_len, "{\"Contrastratio\":10}");
    }
...
/*
 * the handler of rawdata down
 * alink method: thing.model.down_raw
 */
static ssize_t gateway_rawdata_down(const void *in, int in_len, void *out, int out_len, void *ctx)
{
    int index;
...
int main(void)
{
    ...
    /* Subdev PK */
#if (TEST_ERROR_PK == 0)
    char *subdev_pk = SUBDEV_PRODUCTKEY;
#else
    char *subdev_pk = "12345678901";
#endif

    /* Subdev DN */
#if (TEST_ERROR_DN == 0)
    char *subdev_dn = SUBDEV_DEVICENAME;
#else
    char *subdev_dn = "error1";
#endif
