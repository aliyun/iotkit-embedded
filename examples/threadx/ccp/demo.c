...
int ccp_client_demo()
{
    ...
    if (0 != CCPInit(&g_client, &authConfig, COMMAND_TIMEOUT_MS, writeBuf, CCP_WRITE_BUF_LEN,
                     readBuf, CCP_READ_BUF_LEN, onMessage, onConnect, onDisconnect))
    {
    ...
    if (0 != CCPSendDisconnectMsg(&g_client))
    {
        printf("ccp disconnect failed!\n");
    }
