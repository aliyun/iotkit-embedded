...
#define IOTX_HTTP_AUTH_DEVICENAME_STR "{\"version\":\"%s\", \"clientId\":\"%s\",\"signmethod\":\"%s\",\"sign\":\"%s\",\"productKey\":\"%s\",\"deviceName\":\"%s\",\"timestamp\":\"%s\"}"
#else
#define IOTX_HTTP_SIGN_SRC_STR  "clientId%sdeviceName%sproductKey%s"
#define IOTX_HTTP_AUTH_DEVICENAME_STR "{\"version\":\"%s\", \"clientId\":\"%s\",\"signmethod\":\"%s\",\"sign\":\"%s\",\"productKey\":\"%s\",\"deviceName\":\"%s\"}"
#endif
...
iotx_http_context_t * IOT_Http_Init(iotx_device_info_t *p_devinfo)
{
	...
	iotx_device_info_set(p_iotx_http->p_devinfo->product_key, p_iotx_http->p_devinfo->device_name, p_iotx_http->p_devinfo->device_secret);

	return (iotx_http_context_t *)p_iotx_http;
