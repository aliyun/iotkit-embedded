...
int awss_report_statis(const char *module)
{
    ...
    log_content = awss_zalloc(AWSS_STATIS_BUF_LEN + 1);
    if (log_content == NULL) {
        goto STATIS_ERR;
    }
        ...
        case AWSS_STATIS_SM_IDX:
            switch (type) {
                ...
                case AWSS_STATIS_TYPE_TIME_SUC:
                    ...
                    dump_awss_status(STATE_WIFI_STATISTIC, "smartconfig took %d ms, processed %d packs, total len is %d", time,
                                     g_awss_statis.sm.sm_packet_num, g_awss_statis.sm.sm_packet_len_sum);
                    if (SM_SUC > 0) {
