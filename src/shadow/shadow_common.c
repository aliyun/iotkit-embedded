...
#define UPDATE_JSON_STR_END         ",\"clientToken\":\"%s-%d\",\"version\":%d}"
    ...
    char device_id[IOTX_PRODUCT_KEY_LEN + IOTX_DEVICE_NAME_LEN + 2] = {0};
    ...
    IOT_Ioctl(IOTX_IOCTL_GET_PRODUCT_KEY, (void *)product_key);
    IOT_Ioctl(IOTX_IOCTL_GET_DEVICE_NAME, (void *)device_name);
    memcpy(device_id, product_key, strlen(product_key));
    memcpy(device_id + strlen(device_id), ".", strlen("."));
    memcpy(device_id + strlen(device_id), device_name, strlen(device_name));
            ...
            const void *pData)
{
    ...
    if (IOTX_SHADOW_INT32 == type) {
        ret = HAL_Snprintf(buf, buf_len, "%d", *(int32_t *)(pData));
...
#define SHADOW_TOPIC_LEN      (IOTX_PRODUCT_KEY_LEN + IOTX_DEVICE_NAME_LEN)
