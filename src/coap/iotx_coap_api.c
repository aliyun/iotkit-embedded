...
static void iotx_mid_report_response_handler(void *arg, void *p_response)
{
    int len = 0;
    unsigned char *p_payload = NULL;
    char *msg = NULL;
    ...
    IOT_CoAP_GetMessageCode(p_response, &resp_code);
    IOT_CoAP_GetMessagePayload(p_response, &p_payload, &len);
    log_debug("CoAP response code = %d", resp_code);
    log_debug("[CoAP msg_len=%d, msg=%s\r\n", len, p_payload);

    msg = json_get_value_by_name((char *)p_payload, len, "id", &len, 0);
    log_debug("[CoAP mid_report responseID =%s\r\n", msg);
}

/* report ModuleID */
static int iotx_mid_report(iotx_coap_context_t *p_context)
{
...
#define MSG_LEN  (62 + GUIDER_PID_LEN +GUIDER_MID_LEN + 32 +1)
    ...
    log_debug("MID Report starts by CoAP.");
    ...
    message.resp_callback = iotx_mid_report_response_handler;
    ...
    if (IOTX_SUCCESS != (ret = IOT_CoAP_SendMessage(p_context, topic_name, &message))) {
        log_err("send CoAP msg failed%d", ret);
        HAL_Free(msg);
        return FAIL_RETURN;
    }
