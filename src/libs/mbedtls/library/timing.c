...
unsigned long mbedtls_timing_hardclock( void )
{
    ...
    if( hardclock_init == 0 )
    {
        aliyun_iot_timer_start_clock(&tv_init);
        hardclock_init = 1;
    }

    aliyun_iot_timer_start_clock(&tv_cur);
    return (aliyun_iot_timer_interval(&tv_init,&tv_cur) * 1000);
}
...
unsigned long mbedtls_timing_get_timer( struct mbedtls_timing_hr_time *val, int reset )
{
    ...
    if( reset )
    {
        t->start.time  = offset.time;
        return( 0 );
    }

    delta = aliyun_iot_timer_interval(&t->start,&offset);
