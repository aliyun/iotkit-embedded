...
static const char *_sdk_impl_dynamic_register_get_region(void)
{
    sdk_impl_ctx_t *ctx = _iotx_sdk_impl_get_ctx();

    if (ctx->domain_type == 0) {
        return SDK_IMPL_DYNAMIC_REGISTER_REGION_SHANGHAI;
    }else if (ctx->domain_type == 1) {
        return SDK_IMPL_DYNAMIC_REGISTER_REGION_SOUTHEAST;
    }else if (ctx->domain_type == 2) {
        return SDK_IMPL_DYNAMIC_REGISTER_REGION_NORTHEAST;
    }else if (ctx->domain_type == 3) {
        return SDK_IMPL_DYNAMIC_REGISTER_REGION_US_WEST;
    }else if (ctx->domain_type == 4) {
        return SDK_IMPL_DYNAMIC_REGISTER_REGION_EU_CENTRAL;
    }else{
...
static int _sdk_impl_dynamic_register_sign(_IN_ char product_key[PRODUCT_KEY_MAXLEN], _IN_ char product_secret[PRODUCT_SECRET_MAXLEN],
                                            _IN_ char device_name[DEVICE_NAME_MAXLEN], _OU_ char random[SDK_IMPL_DYNAMIC_REGISTER_RANDOM_KEY_LENGTH + 1], _OU_ char sign[SDK_IMPL_DYNAMIC_REGISTER_SIGN_LENGTH])
{
    ...
    res = ubox_random_string(random,SDK_IMPL_DYNAMIC_REGISTER_RANDOM_KEY_LENGTH);
...
static int _sdk_impl_dynamic_register_http(_IN_ char *request_payload, _OU_ char device_secret[DEVICE_SECRET_MAXLEN])
{
    int res = 0;
    ...
    url = _sdk_impl_dynamic_register_get_region();
    if (url == NULL) {
        sdk_err("Invalid Url");
        return FAIL_RETURN;
    }
...
static int _sdk_impl_dynamic_register(_IN_ char product_key[PRODUCT_KEY_MAXLEN], _IN_ char product_secret[PRODUCT_SECRET_MAXLEN],
                                        _IN_ char device_name[DEVICE_NAME_MAXLEN], _OU_ char device_secret[DEVICE_SECRET_MAXLEN])
{
    ...
    char sign[SDK_IMPL_DYNAMIC_REGISTER_SIGN_LENGTH] = {0};
    char random[SDK_IMPL_DYNAMIC_REGISTER_RANDOM_KEY_LENGTH + 1] = {0};
    ...
    /* Calcute Signature */
    res = _sdk_impl_dynamic_register_sign(product_key,product_secret,device_name,random,sign);
    if (res != SUCCESS_RETURN) {
    ...
    dynamic_register_request_len = strlen(dynamic_register_format) + strlen(product_key) + strlen(device_name) + 
                                    strlen(random) + strlen(sign) + strlen(SDK_IMPL_DYNAMIC_REGISTER_SIGN_METHOD_HMACSHA256) + 1;
    ...
    HAL_Snprintf(dynamic_register_request,dynamic_register_request_len,dynamic_register_format,
                    product_key,device_name,random,sign,SDK_IMPL_DYNAMIC_REGISTER_SIGN_METHOD_HMACSHA256);
    
    /* Send Http Request For Getting Device Secret */
    res = _sdk_impl_dynamic_register_http(dynamic_register_request, device_secret);
                      ...
                      void **info_ptr)
{
    ...
    if (ctx->dynamic_register == 0) {
        ...
        rc = _sdk_impl_dynamic_register((char *)product_key,(char *)product_secret,(char *)device_name,device_secret_actual);
