...
static void config_ota_handler(void* pcontext, iotx_cmp_cota_parameter_t* ota_parameter, void* user_data)
{
	iotx_cmp_cota_parameter_t* iotx_cmp_ota_parameter = ota_parameter;
    config_ota_t* config_ota = user_data;

    log_info("\n\n%s\n\n", iotx_cmp_ota_parameter->url);
    /**< save config*/
    ...
    strcpy(config_ota->_rsp_configId, iotx_cmp_ota_parameter->configId);
    strcpy(config_ota->_rsp_getType, iotx_cmp_ota_parameter->getType);
    strcpy(config_ota->_rsp_sign, iotx_cmp_ota_parameter->sign);
    strcpy(config_ota->_rsp_signMethod, iotx_cmp_ota_parameter->signMethod);
    strcpy(config_ota->_rsp_url, iotx_cmp_ota_parameter->url);
...
static void config_ota_cmp_event_handler(void* pcontext, iotx_cmp_event_msg_t* msg, void* user_data)
{
    config_ota_t* config_ota = user_data;
    int ret;
    ...
    if (IOTX_CMP_EVENT_CLOUD_CONNECTED == msg->event_id) {
        if (config_ota->_ota_inited == 0) {
            ...
            ret = IOT_CMP_OTA_Set_Callback(IOTX_CMP_OTA_TYPE_COTA, config_ota_handler, config_ota, NULL);
...
static int config_ota_get(void* _self,const char* configScope, const char* getType, const char* attributeKeys, void* option)
{
	config_ota_t* self = _self;
	self = self;
	return IOT_CMP_OTA_Get_Config(configScope,getType,attributeKeys,option);
}
...
static int config_ota_perform_ota_service(void* _self, void* _data_buf, int _data_buf_length)
{
    ...
    iotx_cmp_ota = config_ota_lite_calloc(1, sizeof(iotx_cmp_ota_t));
	...
	iotx_cmp_ota->ota_type = IOTX_CMP_OTA_TYPE_COTA;
    ...
    while (1) {
        ...
        if (ret == 0 && iotx_cmp_ota->buffer && iotx_cmp_ota->buffer_length) {
            ret = config_ota_write(self, iotx_cmp_ota->buffer, iotx_cmp_ota->buffer_length);
            if (ret == 0) {
        ...
        if (ret || iotx_cmp_ota->result) {
            log_debug("service cota fail, ret=%d,\tresult=%d", ret, iotx_cmp_ota->result);
            goto err_handler;
        }
