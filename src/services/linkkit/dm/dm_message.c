...
static int _dm_msg_property_set_object(int devid, char *key, lite_cjson_t *root)
{
	...
	for (index = 0;index < root->size;index++) {
		...
		if (lite_cjson_is_object(&lite_item_value)) {res = _dm_msg_property_set_object(devid,new_key,&lite_item_value);}
		if (lite_cjson_is_array(&lite_item_value)) {res = _dm_msg_property_set_array(devid,new_key,&lite_item_value);}
		if (lite_cjson_is_number(&lite_item_value)) {res = _dm_msg_property_set_number(devid,new_key,&lite_item_value);}
		if (lite_cjson_is_string(&lite_item_value)) {res = _dm_msg_property_set_string(devid,new_key,&lite_item_value);}
...
int dm_msg_property_set(int devid,dm_msg_request_payload_t *request)
{
	...
	if (lite_cjson_is_object(&lite)) {res = _dm_msg_property_set_object(devid,NULL,&lite);}
...
const char DM_MSG_TOPO_ADD_NOTIFY_USER_PAYLOAD[] DM_READ_ONLY = "{\"result\":%d,\"devid\":%d,\"product_key\":\"%s\",\"device_name\":\"%s\"}";
...
static int _dm_msg_service_set_object(int devid, char *key, lite_cjson_t *root)
{
	...
	for (index = 0;index < root->size;index++) {
		...
		if (lite_cjson_is_object(&lite_item_value)) {res = _dm_msg_service_set_object(devid,new_key,&lite_item_value);}
		if (lite_cjson_is_array(&lite_item_value)) {res = _dm_msg_service_set_array(devid,new_key,&lite_item_value);}
		if (lite_cjson_is_number(&lite_item_value)) {res = _dm_msg_service_set_number(devid,new_key,&lite_item_value);}
		if (lite_cjson_is_string(&lite_item_value)) {res = _dm_msg_service_set_string(devid,new_key,&lite_item_value);}
...
#elif defined (CONFIG_DM_DEVTYPE_GATEWAY)
const char DM_MSG_SERVICE_REQUEST_FMT[] DM_READ_ONLY = "{\"id\":%d,\"devid\":%d,\"serviceid\":\"%.*s\",\"payload\":%.*s}";
#endif
int dm_msg_thing_service_request(_IN_ char product_key[PRODUCT_KEY_MAXLEN], _IN_ char device_name[DEVICE_NAME_MAXLEN], char *identifier, int identifier_len, dm_msg_request_payload_t *request)
{
	...
	if (lite_cjson_is_object(&lite)) {res = _dm_msg_service_set_object(devid,key,&lite);DM_free(key)}
...
#elif defined (CONFIG_DM_DEVTYPE_GATEWAY)
	message_len = strlen(DM_MSG_SERVICE_REQUEST_FMT) + DM_UTILS_UINT32_STRLEN*2 + identifier_len + request->params.value_length + 1;
	message = DM_malloc(message_len);
	if (message == NULL) {
		dm_log_warning(DM_UTILS_LOG_MEMORY_NOT_ENOUGH);
		return FAIL_RETURN;
	...
	memset(message,0,message_len);
	HAL_Snprintf(message,message_len,DM_MSG_SERVICE_REQUEST_FMT,id,devid,identifier_len,identifier,request->params.value_length,request->params.value);
#endif
	res = _dm_msg_send_to_user(IOTX_DM_EVENT_THING_SERVICE_REQUEST,message);
	if (res != SUCCESS_RETURN) {
		DM_free(message);
...
const char DM_MSG_EVENT_THING_DELETE_FMT[] DM_READ_ONLY = "{\"res\":%d,\"productKey\":\"%s\",\"deviceName\":\"%s\",\"devid\":%d}";
...
int dm_msg_thing_sub_register_reply(dm_msg_response_payload_t *response)
{
	...
	for (index = 0;index < lite.size;index++) {
		...
		memset(message,0,message_len);
		HAL_Snprintf(message,message_len,DM_MSG_EVENT_SUBDEV_REGISTER_REPLY_FMT,atoi(temp_id),response->code.value_int,devid);
...
int dm_msg_topo_get_reply(dm_msg_response_payload_t *response)
{
	...
	memset(message,0,message_len);
	HAL_Snprintf(message,message_len,DM_MSG_TOPO_GET_REPLY_FMT,id,response->code.value_int,response->data.value_length,response->data.value);
...
const char DM_MSG_EVENT_SPECIFIC_POST_REPLY_FMT[] DM_READ_ONLY = "{\"id\":%d,\"code\":%d,\"devid\":%d,\"eventid\":\"%.*s\"}";
int dm_msg_thing_event_post_reply(_IN_ char *identifier, _IN_ int identifier_len, _IN_ dm_msg_response_payload_t *response)
{
	...
	memset(message,0,message_len);
	HAL_Snprintf(message,message_len,DM_MSG_EVENT_SPECIFIC_POST_REPLY_FMT,id,response->code.value_int,node->devid,identifier_len,identifier);
...
int dm_msg_combine_login_reply(dm_msg_response_payload_t *response)
{
	...
	memset(message,0,message_len);
	HAL_Snprintf(message,message_len,DM_MSG_EVENT_COMBINE_LOGIN_REPLY_FMT,atoi(temp_id),response->code.value_int,devid);
...
int dm_msg_combine_logout_reply(dm_msg_response_payload_t *response)
{
	...
	memset(message,0,message_len);
	HAL_Snprintf(message,message_len,DM_MSG_EVENT_COMBINE_LOGOUT_REPLY_FMT,atoi(temp_id),response->code.value_int,devid);
...
int dm_msg_thing_model_up_raw_reply(_IN_ char product_key[PRODUCT_KEY_MAXLEN], _IN_ char device_name[DEVICE_NAME_MAXLEN],char *payload, int payload_len)
{
	...
	message_len = strlen(DM_MSG_THING_MODEL_DOWN_FMT) + DM_UTILS_UINT32_STRLEN + payload_len + 1;
	message = DM_malloc(message_len);
	if (message == NULL) {
		dm_log_warning(DM_UTILS_LOG_MEMORY_NOT_ENOUGH);
		return FAIL_RETURN;
	...
	memset(message,0,message_len);
	HAL_Snprintf(message,message_len,DM_MSG_THING_MODEL_DOWN_FMT,devid,payload_len,payload);
...
const char DM_MSG_THING_TOPO_ADD_PARAMS[] DM_READ_ONLY = "[{\"productKey\":\"%s\",\"deviceName\":\"%s\",\"signmethod\":\"%s\",\"sign\":\"%s\",\"timestamp\":\"%s\",\"clientId\":\"%s\"}]";
...
const char DM_MSG_THING_DYNAMICTSL_GET_PARAMS[] DM_READ_ONLY = "{\"nodes\":[\"type\",\"identifier\"],\"addDefault\":false}";
...
const char DM_MSG_COMBINE_LOGIN_PARAMS[] DM_READ_ONLY = "{\"productKey\":\"%s\",\"deviceName\":\"%s\",\"clientId\":\"%s\",\"timestamp\":\"%s\",\"signMethod\":\"%s\",\"sign\":\"%s\",\"cleanSession\":\"%s\"}";
