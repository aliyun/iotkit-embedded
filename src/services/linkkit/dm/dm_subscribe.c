...
static int _iotx_dsub_filter(int devid, int index, int unsub)
{
	...
	/* Check Device Type */
	if ((dcs_mapping[index].dev_type & dev_type) == 0) {return FAIL_RETURN;}
	dm_log_debug("Device Type Matched");
	
	/* Check Service Type */
	if ((dcs_mapping[index].service_type & IOTX_DM_SERVICE_CLOUD) == 0) {return FAIL_RETURN;}
	...
	if (res == SUCCESS_RETURN) {
		if (event_post_reply_opt == 0) {
			if (unsub) {
				...
				res = dm_utils_service_name((char *)IOTX_DCS_SYS_PREFIX,method_reply,product_key,device_name,&unsubscribe);
...
int iotx_dsub_shadow_create(int devid)
{
	...
	if (service_count + event_count == 0) {return SUCCESS_RETURN;}
	...
	for (index = 0;index < service_number;index++) {
		...
		res = dm_utils_service_name((char *)IOTX_DCS_SYS_PREFIX,method,product_key,device_name,(subscribe + service_event_index));
		if (res != SUCCESS_RETURN) {DM_free(method);return FAIL_RETURN;}
	...
	for (index = 0;index < event_number;index++) {
		...
		res = dm_utils_service_name((char *)IOTX_DCS_SYS_PREFIX,method_reply,product_key,device_name,(subscribe + service_event_index));
		if (res != SUCCESS_RETURN) {DM_free(method);return FAIL_RETURN;}
