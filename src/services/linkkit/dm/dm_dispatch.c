...
int dm_disp_uri_prefix_sys_split(_IN_ char *uri, _IN_ int uri_len, _OU_ int *start, _OU_ int *end)
{
	...
	if (memcmp(DM_DISP_SYS_PREFIX,uri,offset+1) != 0) {return FAIL_RETURN;}
...
int dm_disp_uri_prefix_ext_session_split(_IN_ char *uri, _IN_ int uri_len, _OU_ int *start, _OU_ int *end)
{
	...
	if (memcmp(DM_DISP_EXT_SESSION_PREFIX,uri,offset+1) != 0) {return FAIL_RETURN;}
	if (res != SUCCESS_RETURN) {return FAIL_RETURN;}
...
int dm_disp_uri_service_specific_split(_IN_ char *uri, _IN_ int uri_len, _OU_ int *start, _OU_ int *end)
{
	...
	/* Check "thing/service" */
	if (memcmp(DM_DISP_THING_SERVICE_REQUEST,uri + 1,offset) != 0) {return FAIL_RETURN;}
...
int dm_disp_uri_event_specific_split(_IN_ char *uri, _IN_ int uri_len, _OU_ int *start, _OU_ int *end)
{
	...
	/* Check "thing/event/" */
	if (memcmp(DM_DISP_THING_EVENT_POST_REPLY,uri + 1,offset) != 0) {return FAIL_RETURN;}
...
#if 0
int dm_disp_topic_generic_unsubscribe(int devid)
{
	int res = 0, index = 0;
	char product_key[PRODUCT_KEY_MAXLEN] = {0};
	char device_name[DEVICE_NAME_MAXLEN] = {0};
	char *service_name = NULL;
	dm_disp_ctx_t *ctx = _dm_disp_get_ctx();
	dm_disp_topic_mapping_t *dcs_mapping = (dm_disp_topic_mapping_t *)g_dm_disp_topic_mapping;

	if (devid < 0 || index < 0 || index >= sizeof(g_dm_disp_topic_mapping)/sizeof(dm_disp_topic_mapping_t)) {
		dm_log_err(DM_UTILS_LOG_INVALID_PARAMETER);
		return FAIL_RETURN;
	}
	...
	for (index = 0;index < sizeof(g_dm_disp_topic_mapping)/sizeof(dm_disp_topic_mapping_t);index++) {
		service_name = NULL;

		res = dm_utils_service_name((char *)dcs_mapping[index].service_prefix,(char *)dcs_mapping[index].service_name,product_key,device_name,&service_name);
		if (res != SUCCESS_RETURN) {return FAIL_RETURN;}
		...
		if (res == FAIL_RETURN) {dm_log_warning(DM_UTILS_LOG_DMGR_SERVICE_CLOUD_REGISTER_FAILED,strlen(service_name),service_name);}
...
int dm_disp_topic_service_event_unsubscribe(int devid)
{
	...
	for (index = 0;index < service_number;index++) {
		...
		res = dm_utils_service_name((char *)DM_DISP_SYS_PREFIX,method,product_key,device_name,&service_name);
		if (res != SUCCESS_RETURN) {DM_free(method);return FAIL_RETURN;}
...
void dm_disp_thing_event_post_reply(iotx_cm_send_peer_t* source, iotx_cm_message_info_t* msg, char *identifier, int identifier_len, void* user_data)
{
	int res = 0;
	dm_msg_response_payload_t response;
	char int_id[DM_UTILS_UINT32_STRLEN] = {0};
