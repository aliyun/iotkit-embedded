...
int dm_disp_topic_generic_unsubscribe(int devid)
{
    ...
    for (index = 0; index < sizeof(g_dm_disp_topic_mapping) / sizeof(dm_disp_topic_mapping_t); index++) {
        ...
        res = dm_utils_service_name((char *)dcs_mapping[index].service_prefix, (char *)dcs_mapping[index].service_name,
                                    product_key, device_name, &service_name);
        if (res != SUCCESS_RETURN) {
            return FAIL_RETURN;
        }
        ...
        if (res == FAIL_RETURN) {
            dm_log_warning(DM_UTILS_LOG_DMGR_SERVICE_CLOUD_REGISTER_FAILED, strlen(service_name), service_name);
        }
