...
int verify_checksum()
{
    ...
    ret = cal_crc - crc;
    awss_trace("mcast: verify checksum diff is %d, cal_crc, crc is %d,%d\n", ret, cal_crc, crc);
    return ret;
...
int set_zc_bssid()
{
    ...
    if (!memcmp(mcast_smartconfig_data.bssid, (uint8_t *)mcast_bssid_mac + 3, 3)) {
        ...
        awss_trace("mcast_bssid_mac from aplist");
        return SUCCESS_RETURN;
    } else {
#ifdef AWSS_SUPPORT_APLIST
        ...
        if (NULL != ap_info) {
            ...
            awss_trace("bssid1: bssid is %x,%x,%x,%x,%x,%x", zc_bssid[0], zc_bssid[1], zc_bssid[2],
...
int awss_recv_callback_mcast_smartconfig(struct parser_res *res)
{
    ...
    */
    ...
    awss_trace("mcast: index is %d, %d, %d, %d\n", index, payload1, payload2, len);
...
#endif  /* AWSS_SUPPORT_SMARTCONFIG_MCAST */
#endif  /* DISABLE_SMARTCONFIG_MCAST */
