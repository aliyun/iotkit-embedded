...
#if defined(OTA_SUPPORT_DIFF)
    OTA_LOG_I("ota support diff.");
#elif defined(OTA_MD5_CHECK)
                            ...
                            ota_finish_cb_t fcb)
{
    ...
    linkkit_try_leave();
    int count = 0;
    while (!linkkit_is_end() && (count <= 12)) {
        ota_msleep(500);
        count++;
    }
    count = 0;
    extern bool get_awss_notify_running_flag(void);
    while (get_awss_notify_running_flag()&& (count <= 12)) {
        ota_msleep(500);
        count++;
    }
#endif
