...
static code_desc_t sys_depend_desc[] = {
    ...
    {STATE_SYS_DEPEND_TIMER_CREATE, "SDK run into exception when invoking HAL_Timer_Create()"},
    {STATE_SYS_DEPEND_TIMER_START, "SDK run into exception when invoking HAL_Timer_Start()"},
    {STATE_SYS_DEPEND_TIMER_STOP, "SDK run into exception when invoking HAL_Timer_Stop()"},
    {STATE_SYS_DEPEND_TIMER_DELETE, "SDK run into exception when invoking HAL_Timer_Delete()"},
...
static code_desc_t wifi_desc[] = {
    ...
    {STATE_WIFI_STATISTIC, "WiFi configuration statics report, including overall time, pack number, pack lens"},
    {STATE_WIFI_UNEXP_PROT_VERSION, "the version info from the packet is not the expected one"},
    {STATE_WIFI_MCAST_DEBUG_INFO, "WiFi provision debug info during multi-cast wifi-provision"},
    {STATE_WIFI_BCAST_DEBUG_INFO, "WiFi provision debug info during broad-cast wifi-provision"},
